...
import {
  ...
  ModalCloseButton,
  useDisclosure,
  Button, // 💡 Import Button for the Export feature
...
const Globe = () => {
  ...
  // 💡 Change to store an array of selected district IDs for multi-select
  ...
  // 🟢 Function to handle district selection (Toggles multi-select)
  const handleDistrictClick = (district) => {
    ...
    if (isSelected) {
      ...
      // Set map focus to the newly selected district
      setMapFocusDistrict(district); 
    }

    setSelectedDistrictIds(newSelectedIds);

    // 💡 Fetch and show local congregations only for the FIRST selected district 
    // to maintain current UI behavior for the right panel.
    if (newSelectedIds.length > 0) {
      ...
      if (focusDistrict) {
        ...
        axios
          .get(`${API_URL}/api/local-congregations?district_id=${focusDistrict.id}`)
          .then((response) => {
  ...
  const handleCongregationClick = async (cong) => {
    ...
    try {
      const response = await axios.get(
        // The URL needs to match your scraper route
    ...
    if (e.key === "Enter" && searchCongregation.trim() !== "") {
      try {
        ...
        if (allCongregations.length === 0) {
          // You'll need a new backend endpoint for this or use a more targeted one. 
          // Assuming /api/all-congregations exists based on your code logic.
  ...
  // 🟢 New function to handle the export to XLSX
  const handleExport = async () => {
    if (selectedDistrictIds.length === 0) {
      alert("Please select at least one district to export.");
      return;
    }
    ...
    try {
      ...
      const districtsNames = selectedDistrictIds.map(id => 
        districts.find(d => d.id === id)?.name
      ).join('_');
      link.setAttribute('download', `${districtsNames.substring(0, 50)}_Schedule_Export.xlsx`);
      ...
      alert("Export failed. Check the server logs for details.");
    } finally {
    ...
    <Box display="flex" width="100vw" height="100vh">
      ...
      <Box flex="1" position="relative">
        ...
        >
          ...
          />
          {/* Use mapFocusDistrict for flyTo */}
          <MapFlyTo district={mapFocusDistrict} />
          
          {/* 🟢 Fix: Filter out congregations without valid coordinates before mapping */}
          {localCongregations
            .filter(cong => cong.latitude && cong.longitude) // Ensure both exist and are truthy (not null, 0, or undefined)
            .map((cong) => (
              <Marker
                key={cong.id}
